...
class LoanRepayment(models.Model):
    ...
    class Meta:
        ...
        ordering = ['-date']
# Model for logging audit trails of actions taken on various models.
ACTION_CHOICES = [
    ('create', 'Create'),
    ('update', 'Update'),
    ('delete', 'Delete'),
...
class AuditLog(models.Model):
    account = models.ForeignKey(Account,related_name='auditlog', on_delete=models.CASCADE)
    ...
    def __str__(self):
        return f"{self.account} performed {self.action} on {self.model_name} ({self.model_id}) at {self.timestamp}"
    ...
    @staticmethod
    def log_action(account, action, model_name, model_id):
        """Utility method to create an audit log entry."""
        AuditLog.objects.create(
            account=account,
            action=action,
